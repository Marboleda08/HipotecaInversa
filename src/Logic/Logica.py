...
class Mortgage:
    ...
    def verify_value(self):
        if self.property_value < 650000000 or self.property_value > 800000000 or self.property_value < 0:
    ...
    def verify_marital_status(self):
        new_marital_status = self.convert_marital_status()
        if new_marital_status != "single" and new_marital_status != "married":
            raise QuotaError(f"The marital status is incorrect. The error is: {self.marital_status}. The marital status must be single or married")
